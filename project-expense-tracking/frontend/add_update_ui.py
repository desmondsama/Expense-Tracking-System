...
def add_update_tab():
    ...
    with st.form(key="expense_form"):
        ...
        for i in range(5):
            ...
            with col2:
                category_input = st.selectbox(label="Category", options=categories, index=category.index(category),
                                              key=f"category_{i}", label_visibility="collapsed")
            with col3:
                note_input = st.text_input(label="Notes", value=notes, key=f"note_{i}", label_visibility="collapsed")
